...
const FileItem = ({ file, fileType }: TFileItemProps) => {
  ...
  return <div className='w-48 h-48 flex justify-center items-center bg-white rounded-lg border-2'>
    {
      fileType === EFileType.Image ? <img src={sampleImage} alt="Picture" className='w-48 h-48 object-cover rounded-lg' /> : (<div className='flex flex-col item-center justify-center'>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" color="#000000" fill="none" className='mx-auto'>
        ...
        <div className='px-2 w-48 text-center truncate text-ellipsis'>{ file.name }</div>
      </div>)
    }
  </div>
}

const FileUploadButton = ({ uploadFile }: { uploadFile: ChangeEventHandler<HTMLInputElement> }) => {
  return <div>
    <label htmlFor="file-input" className='w-48 h-48 bg-white flex justify-center items-center rounded-lg text-black border-dotted border-2 border-black cursor-pointer'>
    ...
    <input id='file-input' type='file' className='hidden' onChange={uploadFile} />
  </div>
}
...
const PostForm = () => {
  ...
  const determineFileType = (file: File) => {
    ...
    } else if (name.includes("mpeg") || name.includes("wav") || name.includes("ogg") || name.includes("aac")) {
  ...
  const uploadFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < event.target.files.length; i++) {
      ...
      try {
        ...
        console.log(`${file.name} was not processed because file could not be supported.`);
        continue;
      }
    }
  ...
  return <div className="w-7/12">
    ...
    <input className="bg-black text-white p-3 rounded-lg w-full" placeholder='Add title post...'></input>
    <Header text='Description'></Header>
    <EditorContent className="bg-gray-100 p-3 rounded-lg outline-none border-none shadow-none border-0" editor={editor} />
    <Header text='Inspiration'></Header>
    <div className='w-full flex gap-4'>
      <FileUploadButton uploadFile={uploadFile}></FileUploadButton>
      {
        files.map((item) => <FileItem file={item.file} fileType={item.fileType} key={item.file.name} ></FileItem>)
      }
    </div>
    <div className='w-full flex justify-center mt-6'>
      <button className='text-white bg-black p-2 rounded-lg' onClick={createPost}>Create post</button>
    </div>
  </div>
};
